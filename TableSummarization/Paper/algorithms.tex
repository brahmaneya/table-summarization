...
\agp{not essential to mention here.}
\sout{$W$ is required to be non-negative and monotonic. $k$ will usually be small (though the correctness of our solution does not depend on this), since the $k$ rules generated should constitute an easily visualizable summary of the table.}
...
\agp{not essential to mention here}\sout{We will describe a greedy algorithm for finding the approximately highest scoring rule-list.}

First, we show that the rules in the optimal list must effectively be ordered in decreasing order by weight.
\agp{mention that this is independent of MCount. Also give intuition--this is because
we can transfer tuples from one rule to a better rule adn only increase score.}
...
\noindent Thus it is sufficient to restrict our attention to rule-lists that have rules sorted in decreasing order by weight. Or equivalently, given any set $S$ of rules, we can define the Score$(S)$ to be the score of the list obtained by sorting elements of $S$ in decreasing weight order. This gives us 
...
\agp{Confusing since you're conflating lists and sets. Explicitly define the score of a set of rules
is the score of the best rule list (and in the equation above, have max over all R' where R' is an ordered
version of S)}

\noindent The reduction from Problem~\ref{prob:optimal-rule-list} to Problem~\ref{prob:optimal-rule-set} is clear. We now show an approximation algorithm for Problem~\ref{prob:optimal-rule-set}.

\subsection{Algorithm Overview}\label{sec:alg-overview}
\agp{Give a high-level perspective of the algorithm; 
Basics: greedy approximation + a-priori; 
Advanced: Sampling for larger datasets: comprising two parts
how to best use samples, and how to select samples}

\subsection{Greedy Approximation Algorithm}\label{sec:greedy-approx}
\agp{can we show hardness in data or schema complexity?}
...
\agp{Here also, there is a need for some organization. There are clearly
some good ideas here, but you need to give a high-level picture first before
diving in.}
...
\agp{not clear what problem you're trying to solve here. consider raising it up as a subsection

...
\squishend
We have a prototype of the smart drill down operator that is completely implemented and usable. Starting from this point on, whenever we provide result snippets, these will be screenshots from our prototype. In Section~\ref{sec:exp_qualitative}, we will compare smart drill-down with traditional drill-down and show that smart drill-down returns considerably better results; as a matter of fact, smart drill-down can be viewed as a generalization of traditional drill-down (when the weighting function is set to equal $1$ if the drill-down column has a non-$\star$ value and $0$ otherwise), so our implemented prototype also supports traditional drill-down.

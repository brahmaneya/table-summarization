...
Here we set weight equal to the number of non-starred values in the rule $r$ i.e. the {\em size} of the rule. For example, in Table~\ref{table:introexample3}, the rule (Target, bicycles, $\star$) has weight $2$. To get the idea behind using this weighting function, imagine we are trying to reconstruct the table from the rules. We start with a blank value in every column of every table tuple. Then for each displayed rule $r$, we fill each of its values in the corresponding column of $MCount(r)$ tuples. We do this for each rule, filling a total of $\sum_{r} MCount(r)W(r)$ values. Thus maximising score leads to choosing the set of rules that will pre-fill as much of the table as possible.}

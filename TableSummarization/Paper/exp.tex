...
Unless otherwise specified, in all our experiments, we restrict the tables to the first $7$ columns in order to make the result tables fit into the page. We use our current implementation of our algorithms, which directly outputs tables in latex format. We set the $k$ (number of rules) parameter to $4$, and $m_w$ to $5$ for the Size weighting function, and $20$ for the Bits weighting function. Memory capacity $M$ for the SampleHandler is set to $50000$ tuples, and $minSS$ to $5000$.
...
%\hspace{-15pt}
\centering
  \includegraphics[scale=0.6]{graphs/mw_speed.pdf}%change height to 2 inch for actual paper, 5 inch for single column
\vspace{-5pt}

...
\item In Section~\ref{sec:preliminaries}, we introduce the relevant notation and formally define our problem.

\item In Section~\ref{sec:weighting}, we consider different schemes for weighting rules, along with their practical significance.

\item In Section~\ref{sec:interface}, we describe our user interface. 
  
\item In Section~\ref{sec:algorithms}, we present our algorithms for finding optimal rule-based summaries, as well as our sampling schemes for dealing with large tables.

\item In Section~\ref{sec:experiments}, we experimentally evaluate the performance of our algorithms on a real dataset.
...
We choose the $n_{r^{\prime}}$ values so as to maximise the probability that the next user click can be satisfied using samples available in memory. We define some terms required to formalise this problem next. 
...
We now study the effects of the sampling parameter $minSS$. This parameter tells the SampleHandler the minimum sample size on which we run Algorithm~\ref{algo:best-rule-set}.   Higher values of $minSS$ give us more accurate counts for rules, but also increase computation time. 

We consider a value of $minSS$, and a weight function $W$. For those parameters, we expand the empty rule and find the time taken to expand it, as well the the error in counts of the displayed rules. The error is the absolute value of the difference between the actual count of the rule on the entire table, and it's estimated count from the sample. We repeat this $50$ times for each $minSS$ and $W$, and find the average time and error per $minSS$ and $W$. The average time over these $50$ iterations is plotted against $minSS$, for $W(r) = \text{Size}(r)$ and $W(r) = \sum_{c \in C : r(c) \neq \star} \lceil \text{log}_2(|c|) \rceil$ in Figure~\ref{fig:minSS_speed}. The average error is plotted against $minSS$, for $W(r) = \text{Size}(r)$ and $W(r) = \sum_{c \in C : r(c) \neq \star} \lceil \text{log}_2(|c|) \rceil$ in Figure~\ref{fig:minSS_error}.
...
\begin{figure}
\hspace{-20pt}
  \includegraphics[height=2in]{graphs/minSS_error_count.pdf}%change height to 2 inch for actual paper, 5 inch for single column
\vspace{-10pt}
  \caption{Error in Count for different values of parameter $minSS$ \label{fig:minSS_error}}
\vspace{-13pt}
\end{figure}

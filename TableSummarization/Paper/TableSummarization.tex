...
% TODO: Do and mention that we do Experiments to show performance/usefulness.
\end{abstract}
...
% Something about data being ubiquitous. Data is only as valuable as the insights we can get from it. Data analysts need to sift through large amounts of data to learn about trends and obtain insights from it. Big amounts of data hard for human to analyze, making it necessary to summarize the data. The summary needs to be fast and interactive to facilitate exploration. Help users make sense of data. 
% Mention our problem. Then features. 

% TODO : Write introduction containing points below. 
% say that paper is on summarization. 
% Salient features 
% Interactive : = Dynamic + Real-Time (see below) 
% Flexible/Customizable : Because user can choose size/ weighting function (which columns/rule sizes to emphasize).
% Dynamic : Because it modifies according to user clicks.
% Real time : Because time taken is very small compared to explananion tables (and cubes?). 
% Coverage : We emphasize coverage over surprise. (Need motivation for this). Also motivate coverage based scoing over only giving top scoring rules i.e. MCount over Count.
% Interpretable : Meaning of rules is easy to understand. (But their significance? Not sure about this).
% Compare to existing work. OLAP/Explanation tables take more time, don't focus on coverage. OLAP requires more user clicks to get same info. Explanation tables is not interactive (verify this). Weighting functions also make our system highly flexible, allowing users to prefer rules of certain sizes, or with certain columns. 
...
% TODO: Overview of paper and list of contributions
...
%TODO: Read and improve flow of this section.
...
% Need to find and write about the exact algorithm to decide how much to load, what to shrink, and how much to shrink.

\subsubsection{Setting $minSS$}
% TODO: Write bound on reasonable value of minSS

% Additional optimizations. In sample tuples, store only those columns that are not in filter rule. If tuples are repeated across samples, then store pointer to the tuple instead. 

%Independent. Can we bunch together rare values into a single value called 'rare value'?

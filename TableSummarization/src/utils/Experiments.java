...
public class Experiments {
	...
	public static void minSSUSCensusErrorTest (TableInfo table, List<Integer> columns, int mw, int minSSMin, int minSSMax, int minSSStep, int ruleNums, Scorer scorer, String outFile) throws IOException {
		int capacity = Integer.MAX_VALUE;
		PrintWriter pw = new PrintWriter(new FileWriter(outFile));
		Rule emptyRule = new Rule(new HashMap<Integer, Integer>(), table.dictionary.size());
		int numIters = 50;
		...
		Set<Rule> selectedRules = new HashSet<Rule>();
		{
			...
			RuleTree ruleTree = new RuleTree(table);
			ruleTree.expandRow(emptyRule, 2 * ruleNums, mw, scorer, sampleHandler);
			Set<Rule> candidateRules = new HashSet<Rule>();
			for (RuleNode child : ruleTree.root.children) {
				final Rule childRule = child.rule;
				childRule.count = 0;
				candidateRules.add(childRule);
			}
			Long totalNumTuples = USCensus1990.updateRuleCounts(table, columns, candidateRules);
			for (Rule rule : candidateRules) {
				Integer scaledCount = (int)(1.0 * rule.count * table.contents.size() / (totalNumTuples + 0.0));
				ruleCounts.put(rule, scaledCount);	
			}
			ruleTree.contractRow(emptyRule);
			ruleTree.expandRow(emptyRule, ruleNums, mw, scorer, sampleHandler);
			for (RuleNode child : ruleTree.root.children) {

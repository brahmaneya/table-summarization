...
In fact, if we assume that values in columns occur independently of each other, and that the most frequent value occurs $x < 1$ fraction of the time, then the number of candidate rules is bounded by $c_i \leq c_1 x^i$. This gives us $\sum_i c_i \leq c_1\sum_i x^i \leq c_1/(1-x)$.
...
%%%%%%%%%%%%%%%%%%
While the setting of $W$ (and hence $m_w$ is mostly specific to the user, we will consider a simple model of how frequently values occur in a table and determine a weighting function to favor rules of a particular size based on that model. 

Let us consider a parametric weight function which generalizes the `Bits Weighting Function' from Section~\ref{sec:weighting}. 
$W(r) = \left(\sum_{c \in C} o_{r,c}w_c\right)^k$, where the $w_c$'s and $k$ are parameters of the weighting function, and $o_{r,c}$ ($o$ stands for occurrence) is $1$ if $r(c) \neq \star$ and $0$ otherwise. The Size and Bits weighting functions can be seen as special cases of the above function with $k=1$ and with $w_c$ equal to $1$ for Size and $\text{log}(|c|)$ for Bits.

...
\begin{comment}
\papertext{$W(r) = |\left\lbrace c \in C \mid r(c) \neq \star \right\rbrace |$ : 
...
%\papertext{ As an example, clicking on the trivial rule of Table~\ref{table:introexample0} would display Table~\ref{table:introexample}. Clicking further on the third rule in the expanded rule-list would display Table~\ref{table:introexample2}.} 
...
Now let $S \subsetneq S'$ and $s \notin S$. Then to prove submodularity, we simply need to prove that $$\text{Score}(S \cup \{s\}) - \text{Score}(S) \geq \text{Score}(S' \cup \{s\}) - \text{Score}(S')$$. This gives us
...
\eat{We present the idea behind the approximate solution below. A more detailed description can be found in the technical report. The solution consists of two main steps:
\squishlist
...
Next, we combine the solutions using dynamic programming. Let the $M$ sets be called $M_0, M_1, ... M_D$, while possible sample sizes range from $0$ to $\mathcal{S}$. The number of sample sizes can be pretty large, but we can make it smaller by discretizing, say to have granularity $100$. Then we create a $D \times \mathcal{S}$ array $A$. The value $A\left[i\right]\left[j\right]$ contains the best probability score we can get from $M_0, M_1, ... M_i$ with total sample size at most $j$. We can populate $A\left[0\right]\left[j\right] \forall j$ using the locally optimal solutions for $M_0$. Let $E_{i+1}$ denote the set of locally optimal solutions for $M_{i+1}$. Then we have,
...
Dynamic programming solves this recurrence in $O(D\mathcal{S}3^d)$ time. 
}
...
annual household income, gender, marital status, age, education, occupation, time lived in the Bay Area, dual incomes status, persons in household, persons in household under $18$, householder status, type of home, ethnic classification, and language most spoken in home.
...
Unless otherwise specified, in all our experiments, we restrict the tables to the first $7$ columns in order to make the result tables fit in the page. We use the current implementation of our smart drill down operator, and insert cropped screenshots of its output in this paper. 
...
\begin{figure}
\centering
\includegraphics[width=80mm,frame]{graphs/screenshots6col/empty_expansion.png}
...
%\vspace{-10pt}
\centering
\includegraphics[width=80mm,frame]{graphs/screenshots6col/rule_expansion.png}
...
\begin{figure}
\centering
\includegraphics[width=80mm,frame]{graphs/screenshots6col/age_drilldown.png}
...
\begin{figure}
\centering
\includegraphics[width=80mm,frame]{graphs/screenshots6col/bits_scoring.png}
...
\vspace{-10pt}
\centering
\includegraphics[width=80mm,frame]{graphs/screenshots6col/min2_scoring.png}
...
\papertext{
\subsection{Setting parameters $k$, $m_w$, $W$}\label{sec:setting-paramters}
...
$W$ is the weight function that determines which rules are interesting. Specifying an arbitrary function can be hard, so instead we hardcode some common Weight functions and allow the user to choose one from a drop-down menu.

\noindent
\textbf{Generalizing our weight functions:} We now analyze a parametric family of weighting functions that generalizes our functions from Section~\ref{sec:weighting} and provides intuition for them. Let $W(r) = \left(\sum_{c \in C} o_{r,c}w_c\right)^{\mathsf{p}}$, where the $w_c$'s and $\mathsf{p}$ are parameters of the weighting function, and $o_{r,c}$ ($o$ stands for occurrence) is $1$ if $r(c) \neq \star$ and $0$ otherwise. The Size and Bits weighting functions can be seen as special cases of the above function with $\mathsf{p}=1$ and with $w_c$ equal to $1$ for Size and $\text{log}(|c|)$ for Bits.
...
$$S(r) = \left(\sum_{c \in C} o_{r,c}w_c\right)^{\mathsf{p}}|T|\Pi_{c \in C}f_c^{o_{r,c}}$$
...
We can also estimate the size of the optimal rule under the above weighting function. Specifically, the weighted fraction of columns with non-$\star$ values is given by $\frac{\sum_{c\in C} o_{r,c}w_c}{\sum_{c \in C} w_c}$ which equals $-\mathsf{p}/(\sum_{c \in C} \ln f_c)$. Thus, if we wanted our highest score rule to have $s$ fraction of the columns get instantiated, we can achieve that by setting $\mathsf{p} = -s\sum_{c \in C} \ln f_c$.

The above reasoning also lets us estimate the $m_w$ parameter we need to use. Specifically, the highest score rule above has weight $(\frac{-\ln f_c}{\mathsf{p}w_c})^{\mathsf{p}}$.
...
Our framework assumes that all attributes are categorical. It does not summarize numerical values. But it can easily be modified to do so by bucketizing a numerical attribute and treating the bucket id as a categorical attribute. This is already done in our MD dataset, where numerical attributes like age are divided into buckets ($18-24$, $25-34$ and so on).


\vspace{-5pt}
\subsection{Using Sum instead of Count}\label{sec:extensions-sum}
We defined the total score of a rule-list using the marginal counts of rules in the list, and display the counts of rules in our table summary. However, if we have a numerical (i.e. a `measure') column in the table, it is straightforward to extend our summary to the `Sum' aggregate over that column. We can define $Sum$ and $MSum$ analogously to $Count$ and $MCount$, and $Score$ can be modified to use $MSum$ instead of $MCount$. We can then modify Algorithm~\ref{algo:best-rule-set} to maximize the new score by replacing $Count(r)$ by $Sum(r)$ and computing sum and marginal sum instead of count and marginal count in each pass over the table.}

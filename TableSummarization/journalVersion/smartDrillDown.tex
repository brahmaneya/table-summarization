...
\mrj{Our algorithm calls the function for finding best marginal rule $k$ times. Thus the total computational cost is about $k$ times the cost of computing the best marginal rule. Let $c_n$ be the number of counts in $C_n$ at the end of the $n^{th}$ iteration (that is, after eliminating rules that have no chance of having the highest marginal value). Also, let $c_0 = 1$.}
